...
mutation.plot.transcripts <- function(bottom,start, stop, text="", drop=-0.15, col="red", haplotypes=NULL,text_offset=10,lwidth= 0.4){
  rect(start, bottom, stop, drop - bottom +.01*length(haplotypes), col=col, border=col,lwd=lwidth)
  text(stop + text_offset, drop - bottom, text, cex=0.7, col=col, pos=4, offset=0.1+.1*length(haplotypes))
  for (i in 1:length(haplotypes)) points(stop, drop-0.005+(i-1)*0.1, col=haplotypes[i], pch=20, cex=2)
}
...
t_no <- function(t){
  as.numeric(stringr::str_split_fixed(t,"\\.",2)[2])
}
                               ...
                               legend_gap)
{
  ...
  if (nrow(transcripts_filtered) == 0){
    return()
  }
  max_transcripts<- max(unlist(lapply(transcripts_filtered$transcript,t_no)))
  
  Transcript.model.browser.plot(model=transcripts_filtered, gene_info=filtered_genes,max_transcripts= max_transcripts, gene_start=plot_start, gene_bpstop=plot_end, xaxis=T, legend_gap=legend_gap, gap=0.2)
