	...
	def get_dn_ds(self):
		...
		for gene, position_annotation in self.Instances_clean.items():
			...
			if n and s:
				...
				self.dn_ds[gene] = "NA"
	#
	def write_snp_toRscript(self, gene, transcript, Rout, snpeffout):
...
def write_R_script(outdir, Routfile, gene_output, domain_output, snpeff, annotation, interproscan, filetype):
	...
	with open(Routfile, "w") as Rout, open(outdir + "Shiny_out.csv", "w") as shinyout, open(outdir + "snpeff_out.csv","w") as snpeffout:
		...
		#
		#
		Rout.write("library(dplyr)\n\nsource('genemodel_adjusted.r')\n\n")
		#
		Rout.write(f"input <- '{gene_output}'\n")
		Rout.write(f"transcripts <- read.csv(input)\n\n")
		#
		#
		Rout.write(f"domain_input <- '{domain_output}'\n")
		Rout.write(f"domains <- read.csv(domain_input)\n\n")
		#
		for gene, transcripts in annotation.gene_transcripts.items():
			...
			Rout.write("transcript_vector <- as.vector(unique(gene$transcript))\n")
			Rout.write(f"gene.transcript.model.plot_domain(model=g_domains, transcript_vector=transcript_vector, gene_start={gene_start}, gene_bpstop={gene_end}, orientation='{orientation}', gap=0.2)\n\n")
			#
			#pdb.set_trace()
			transcripts = list(transcripts) #with gtf transcripts are a set
			transcripts.sort()#So we go through .1 -> .n
			for transcript in transcripts:
				Rout.write(f"#Working on {transcript}\n\n\n")
				#
				transcript_start, transcript_end = annotation.write_genemodel(gene, transcript, gene_output)
				#
				Rout.write(f"transcript <- transcripts %>% filter(transcript == '{transcript}')\n\n")
				Rout.write(f"genemodel.plot(model=transcript, start={transcript_start}, bpstop={transcript_end}, orientation='{orientation}', xaxis=T)\n\n")
				#
				if interproscan:
					try:
						...
						Rout.write(f"t_domains <- domains %>% filter(transcript == '{transcript}')\n")
						Rout.write(f"genemodel.plot_domain(model=t_domains, start={transcript_start}, bpstop={transcript_end}, orientation='{orientation}')\n\n")
					except KeyError:
						Rout.write(f"#No domains for {transcript}\n\n")
				#
				if snpeff:
					snpeff.write_snp_toRscript(gene, transcript, Rout, snpeffout)
